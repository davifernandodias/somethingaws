    ...
    switch (action.type) {
        ...
        case 'controls_quantity_selection_alternatives': {
            ...
            if (current.includes(index)) {
                return {...state, selectedAnswers: { ...state.selectedAnswers, [questionId]: current.filter((i: number) => i !== index) }};
            }
            if (current.length >= max) { return state; }

            return {...state, selectedAnswers: { ...state.selectedAnswers, [questionId]: [...current, index] }};
        }
